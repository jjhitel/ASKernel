...
static bool wakeup_source_blocker(struct wakeup_source *ws)
{
	...
	if (ws) {
		...
		if ((!enable_sensorhub_wl && !strncmp(ws->name, "SENSORHUB", wslen)) ||
 ...
 * knowsedge, however, may not be available to it, so it can simply specify time
